export default {
	provineList:    [{ "idProvince": "01", "name": "Thành phố Hà Nội" },
	...
	getLocation() {
		appsmith.geolocation.getCurrentPosition(async (location)=>{
			storeValue("userLocation", location.coords);
			// await geo_location.run({
			// latlng: [location.coords.latitude, location.coords.longitude].join(", ")
			// });
		})
	}
}